#ifndef GPHOTO2PP_TESTS_CAMERAFILEWRAPPER_GENERIC_H_  // NOLINT
#define GPHOTO2PP_TESTS_CAMERAFILEWRAPPER_GENERIC_H_
/** \file
...
class CameraFileWrapper_NoDevice : public CxxTest::TestSuite {
 ...
 public:
    ...
    void testInitializeFile() {
        // Now we don't necessarily need to get a legitimate file (could read one from disk or use a dummy one),
        // but I prefer to get a file from the camera to do all the tests.
        ...
        if (in) {
            // ~ std::vector<char> fileContents((std::istreambuf_iterator<char>(in)), std::istreambuf_iterator<char>());
            // This is a shorter and cleaner way, but will resize the vector as it's made.
    ...
    void testMoveAssignment() {
        ...
        TS_ASSERT_THROWS(_file.getFileName(), const gphoto2pp::exceptions::gphoto2_exception&);
    ...
    void testMoveConstructor() {
        ...
        TS_ASSERT_THROWS(_file.getFileName(), const gphoto2pp::exceptions::gphoto2_exception&);
        // The old value is no longer valid again

        _file = std::move(moveconstructor);  // now copy it back, ironically using move assignment which we previously
    ...
    void testMimeFunctions() {
        _file.detectMimeType();  // By default it will be unknown/unknown, this will now change it to hopefully
                                 // be image/png
    ...
    void testTimeAccessors() {
        // I don't have any better way to test the time set and get for now. I just read, write it back with 5 seconds
        // later, and then read and diff the two.
